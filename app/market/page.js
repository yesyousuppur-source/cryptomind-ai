...
export default function MarketPage() {
  ...
  const [sort,   setSort]      = useState("volume"); // volume | ch24 | price | name
  ...
  useEffect(() => {
    if (autoRef) {
      timerRef.current = setInterval(fetchMarket, 30000);
    }
  ...
  return (
    <main style={{fontFamily:"'Inter',sans-serif",background:"#f0fdf8",minHeight:"100vh",padding:"0 0 40px"}}>
      ...
      <div style={{background:"linear-gradient(135deg,#0f172a,#1e293b)",padding:"16px 16px 20px",marginBottom:0}}>
        ...
        <p style={{fontSize:12,color:"#64748b",marginBottom:14}}>Top 120 coins — Binance se live data · Har 30 sec refresh</p>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          <button onClick={()=>setAutoRef(p=>!p)}
            style={{width:40,height:22,borderRadius:100,border:"none",cursor:"pointer",
              background:autoRef?"#10b981":"#475569",position:"relative",transition:"all .3s",flexShrink:0}}>
            <div style={{position:"absolute",top:3,width:16,height:16,borderRadius:"50%",background:"#fff",
              transition:"all .3s",left:autoRef?"21px":"3px",boxShadow:"0 1px 3px rgba(0,0,0,.3)"}}/>
          </button>
          <span style={{fontSize:11,color:"#64748b"}}>Auto refresh {autoRef?"ON — har 30 sec":"OFF"}</span>
          <button onClick={fetchMarket} style={{marginLeft:"auto",background:"rgba(16,185,129,.2)",border:"1px solid rgba(16,185,129,.4)",borderRadius:20,padding:"4px 12px",fontSize:11,color:"#6ee7b7",fontWeight:600,cursor:"pointer"}}>🔄 Now</button>
        </div>
      </div>
