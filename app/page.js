        ...
        {activeTab==="analyze" && (
          <div className="fadein">
            ...
            <div className="hov" style={{...CARD,position:"relative",overflow:"hidden"}}>
              ...
              {!top5Fetched?(
                <div style={{textAlign:"center"}}>
                  <p style={{fontSize:13,color:"#64748b",marginBottom:8}}>25 coins scan → RSI + MA50 + MA200 strict analysis</p>
                  <div style={{background:"#f0fdf4",border:"1px solid #6ee7b7",borderRadius:10,padding:"8px 12px",fontSize:11,color:"#059669",marginBottom:12}}>
                    ✅ Sirf 65+ score wale coins dikhenge — quality over quantity
                  </div>
                  <button className="btn" onClick={fetchTop5} style={{padding:"11px 24px",fontSize:13,borderRadius:12}}>🔍 Scan Strong Signals</button>
                </div>
              ):top5Load?(
                <div style={{textAlign:"center",padding:"16px 0"}}>
                  <div style={{display:"flex",justifyContent:"center",gap:10,marginBottom:10}}>
                  ...
                  <p style={{fontSize:12,color:"#64748b"}}>Strict filter se 25 coins scan ho rahe hain…</p>
                </div>
              ):top5?.coins?.length>0?(
                <div>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                  ...
                  {/* ⚠️ IMPORTANT WARNING before signals */}
                  <div style={{background:"linear-gradient(135deg,#fff7ed,#fed7aa)",border:"1px solid #fdba74",borderRadius:12,padding:"12px 14px",marginBottom:12}}>
                    <div style={{fontWeight:700,fontSize:12,color:"#c2410c",marginBottom:6}}>⚠️ Invest Karne Se Pehle Padho</div>
                    <div style={{fontSize:11,color:"#9a3412",lineHeight:1.7}}>
                      • Sirf woh paisa lagao jo <strong>aap lose kar sako</strong><br/>
                      • Ek coin mein <strong>max 10-15%</strong> portfolio lagao<br/>
                      • Stop loss <strong>hamesha set karo</strong> — neeche diya hai<br/>
                      • Yeh <strong>financial advice nahi</strong> — DYOR karo
                    </div>
                  </div>

                  <div style={{display:"flex",flexDirection:"column",gap:12}}>
                    {top5.coins.map((coin,i)=>{
                      // Calculate stop loss (6%) and target (12%) from price
                      const priceNum = parseFloat(coin.price.replace(/[$,]/g,""));
                      const stopLoss = (priceNum * 0.94).toLocaleString(undefined,{maximumFractionDigits:4});
                      const target1  = (priceNum * 1.08).toLocaleString(undefined,{maximumFractionDigits:4});
                      const target2  = (priceNum * 1.15).toLocaleString(undefined,{maximumFractionDigits:4});
                      return(
                      <div key={coin.symbol} style={{background:"#fff",border:`2px solid ${coin.signalColor}33`,borderRadius:16,overflow:"hidden",boxShadow:"0 4px 16px rgba(0,0,0,.06)"}}>
                        {/* Header */}
                        <div style={{background:`linear-gradient(135deg,${coin.signalBg},#fff)`,padding:"12px 14px",display:"flex",alignItems:"center",gap:10,borderBottom:"1px solid #f1f5f9"}}>
                          <div style={{width:28,height:28,borderRadius:"50%",background:i===0?"linear-gradient(135deg,#fbbf24,#f59e0b)":i===1?"linear-gradient(135deg,#94a3b8,#64748b)":i===2?"linear-gradient(135deg,#cd7c32,#b45309)":"linear-gradient(135deg,#e2e8f0,#cbd5e1)",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:900,fontSize:11,color:i<3?"#fff":"#64748b",flexShrink:0}}>{i+1}</div>
                          <img src={coin.image} alt="" onError={e=>e.target.style.display="none"} style={{width:32,height:32,borderRadius:"50%",border:"2px solid #e2e8f0",flexShrink:0}}/>
                          <div style={{flex:1}}>
                            <div style={{fontWeight:800,fontSize:14}}>{coin.name}</div>
                        ...
                        {/* Signal Badge */}
                        <div style={{padding:"8px 14px",borderBottom:"1px solid #f1f5f9",display:"flex",gap:6,alignItems:"center"}}>
                          <span style={{background:coin.signalBg,border:`1px solid ${coin.signalColor}44`,borderRadius:20,padding:"3px 12px",fontSize:11,color:coin.signalColor,fontWeight:800}}>{coin.signal}</span>
                          <span style={{background:coin.risk==="Low"?"#f0fdf4":coin.risk==="Medium"?"#fffbeb":"#fef2f2",border:`1px solid ${coin.risk==="Low"?"#6ee7b7":coin.risk==="Medium"?"#fde68a":"#fca5a5"}`,borderRadius:20,padding:"3px 10px",fontSize:10,color:coin.risk==="Low"?"#059669":coin.risk==="Medium"?"#d97706":"#dc2626",fontWeight:700}}>Risk: {coin.risk}</span>
                          {coin.rsi!=="—"&&<span className="mono" style={{fontSize:10,color:"#94a3b8"}}>RSI: {coin.rsi}</span>}
                        </div>

                        {/* Why this signal */}
                        {coin.reason&&(
                          <div style={{padding:"8px 14px",borderBottom:"1px solid #f1f5f9",background:"#fafafa"}}>
                            <div style={{fontSize:10,color:"#94a3b8",fontWeight:600,marginBottom:3}}>WHY THIS SIGNAL</div>
                            <div style={{fontSize:11,color:"#475569",lineHeight:1.5}}>📊 {coin.reason}</div>
                          </div>
                        )}

                        {/* Stop Loss + Targets — MOST IMPORTANT */}
                        <div style={{padding:"10px 14px"}}>
                          <div style={{fontSize:10,color:"#94a3b8",fontWeight:600,marginBottom:8,letterSpacing:.5}}>🛡️ RISK MANAGEMENT</div>
                          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:6}}>
                            <div style={{background:"linear-gradient(135deg,#fef2f2,#fee2e2)",borderRadius:10,padding:"8px 6px",textAlign:"center",border:"1px solid #fca5a5"}}>
                              <div style={{fontSize:9,color:"#dc2626",fontWeight:700,marginBottom:3}}>🛑 STOP LOSS</div>
                              <div className="mono" style={{fontSize:10,fontWeight:800,color:"#991b1b"}}>${stopLoss}</div>
                              <div style={{fontSize:8,color:"#dc2626",marginTop:2}}>−6% pe exit karo</div>
                            </div>
                            <div style={{background:"linear-gradient(135deg,#f0fdf4,#dcfce7)",borderRadius:10,padding:"8px 6px",textAlign:"center",border:"1px solid #6ee7b7"}}>
                              <div style={{fontSize:9,color:"#059669",fontWeight:700,marginBottom:3}}>🎯 TARGET 1</div>
                              <div className="mono" style={{fontSize:10,fontWeight:800,color:"#065f46"}}>${target1}</div>
                              <div style={{fontSize:8,color:"#059669",marginTop:2}}>+8% profit book karo</div>
                            </div>
                            <div style={{background:"linear-gradient(135deg,#eff6ff,#dbeafe)",borderRadius:10,padding:"8px 6px",textAlign:"center",border:"1px solid #93c5fd"}}>
                              <div style={{fontSize:9,color:"#2563eb",fontWeight:700,marginBottom:3}}>🚀 TARGET 2</div>
                              <div className="mono" style={{fontSize:10,fontWeight:800,color:"#1d4ed8"}}>${target2}</div>
                              <div style={{fontSize:8,color:"#2563eb",marginTop:2}}>+15% strong hold</div>
                            </div>
                          </div>

                          {/* Position Size Warning */}
                          <div style={{marginTop:8,background:"linear-gradient(135deg,#fffbeb,#fef3c7)",borderRadius:8,padding:"7px 10px",display:"flex",gap:6,alignItems:"center"}}>
                            <span style={{fontSize:14}}>💡</span>
                            <div style={{fontSize:11,color:"#92400e",lineHeight:1.5}}>
                              <strong>Position Size:</strong> Is coin mein apne portfolio ka max <strong>10-15%</strong> lagao.
                              Agar stop loss hit ho → <strong>turant exit karo.</strong>
                            </div>
                          </div>
                        </div>
                      </div>
                  ...
                  {/* Footer disclaimer */}
                  <div style={{marginTop:12,background:"#f8fafc",borderRadius:10,padding:"10px 12px",fontSize:10,color:"#94a3b8",textAlign:"center",lineHeight:1.7}}>
                    ⚠️ Yeh signals technical analysis pe based hain — guaranteed profit nahi.<br/>
                    Crypto highly volatile hai. Hamesha DYOR (Do Your Own Research) karo.
                  </div>
                </div>
              ):(
                <div style={{background:"linear-gradient(135deg,#fffbeb,#fef3c7)",border:"1px solid #fde68a",borderRadius:14,padding:"14px 16px"}}>
                  ...
                  <div style={{fontSize:12,color:"#78350f",lineHeight:1.7,marginBottom:10}}>
                    {top5?.message==="no_signal"
                      ?"Humara strict filter abhi kisi coin ko safe buy nahi bol raha. Market conditions weak hain — yeh acha sign hai ki tum galat entry se bach rahe ho. Kal dobara check karo."
                      :"Market mein abhi limited opportunities hain. Jo dikh raha hai woh genuinely strong signal hai."}
                  </div>
                  <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:10}}>
                    {["RSI oversold nahi","MA support nahi","Pehle pump ho chuka","Volume suspicious"].map((r,i)=>(
                      <span key={i} style={{background:"rgba(0,0,0,.06)",borderRadius:20,padding:"3px 10px",fontSize:10,color:"#92400e"}}>❌ {r}</span>
                    ))}
                  </div>
                  <div style={{display:"flex",gap:8}}>
                    <button onClick={fetchTop5} style={{flex:1,background:"#f59e0b",color:"#fff",border:"none",borderRadius:10,padding:"8px",fontWeight:700,fontSize:12,cursor:"pointer",fontFamily:"'Inter',sans-serif"}}>
                      🔄 Dobara Scan Karo
                    </button>
                    <div style={{flex:1,background:"#fff",borderRadius:10,padding:"8px",fontSize:11,color:"#92400e",fontWeight:600,textAlign:"center",display:"flex",alignItems:"center",justifyContent:"center"}}>
                      ⏰ 10 min mein auto-update
                    </div>
                  </div>
                </div>
              )}
            ...
            {/* ── QUICK TOOLS — always visible below search ── */}
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12}}>
              {/* Price Alert quick */}
              <div style={{background:"linear-gradient(135deg,#fffbeb,#fef3c7)",border:"2px solid #fde68a",borderRadius:16,padding:"12px 14px",cursor:"pointer"}}
                onClick={()=>document.getElementById("price-alert-section")?.scrollIntoView({behavior:"smooth"})}>
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}>
                  <span style={{fontSize:20}}>🔔</span>
                  <span style={{fontWeight:700,fontSize:13,color:"#92400e"}}>Price Alert</span>
                </div>
                <div style={{fontSize:10,color:"#d97706"}}>Target set karo</div>
                {alerts.length>0&&<div style={{marginTop:4,background:"#f59e0b",borderRadius:20,padding:"1px 8px",fontSize:9,fontWeight:700,color:"#fff",display:"inline-block"}}>{alerts.length} Active</div>}
              </div>
              {/* Screenshot quick */}
              <div style={{background:"linear-gradient(135deg,#ecfdf5,#d1fae5)",border:"2px solid #6ee7b7",borderRadius:16,padding:"12px 14px",cursor:"pointer"}}
                onClick={()=>document.getElementById("screenshot-section")?.scrollIntoView({behavior:"smooth"})}>
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}>
                  <span style={{fontSize:20}}>📸</span>
                  <span style={{fontWeight:700,fontSize:13,color:"#065f46"}}>Screenshot</span>
                </div>
                <div style={{fontSize:10,color:"#059669"}}>Portfolio analyze karo</div>
              </div>
              {/* Personal Advisor quick */}
              <div style={{background:"linear-gradient(135deg,#fef2f2,#fee2e2)",border:"2px solid #fca5a5",borderRadius:16,padding:"12px 14px",cursor:"pointer"}}
                onClick={()=>document.getElementById("advisor-section")?.scrollIntoView({behavior:"smooth"})}>
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}>
                  <span style={{fontSize:20}}>🎯</span>
                  <span style={{fontWeight:700,fontSize:13,color:"#991b1b"}}>Advice</span>
                </div>
                <div style={{fontSize:10,color:"#dc2626"}}>Personal suggestion lo</div>
              </div>
              {/* Budget quick */}
              <div style={{background:"linear-gradient(135deg,#eff6ff,#dbeafe)",border:"2px solid #93c5fd",borderRadius:16,padding:"12px 14px",cursor:"pointer"}}
                onClick={()=>document.getElementById("budget-section")?.scrollIntoView({behavior:"smooth"})}>
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}>
                  <span style={{fontSize:20}}>💰</span>
                  <span style={{fontWeight:700,fontSize:13,color:"#1e40af"}}>Budget</span>
                </div>
                <div style={{fontSize:10,color:"#2563eb"}}>Best coins suggest karo</div>
              </div>
            </div>
            ...
            {/* ── PRICE ALERT ── */}
            <div id="price-alert-section" className="hov" style={{...CARD,border:"2px solid #fde68a",background:"linear-gradient(135deg,#fff,#fffbeb)"}}>
              <SH icon="🔔" title="Price Alert" subtitle="Target price set karo — hum yaad dilayenge" bg="linear-gradient(135deg,#fffbeb,#fef3c7)" br="#fde68a"/>
              <div style={{display:"flex",gap:8,marginBottom:10}}>
                ...
                <select value={alertType} onChange={e=>setAlertType(e.target.value)}
                  style={{background:"#f8fafc",border:"2px solid #e2e8f0",borderRadius:12,padding:"10px",fontSize:12,color:"#0f172a",fontFamily:"'JetBrains Mono',monospace",cursor:"pointer"}}>
              ...
              <div style={{display:"flex",gap:8,marginBottom:10}}>
                <input value={alertPrice} onChange={e=>setAlertPrice(e.target.value)}
                  placeholder="Target price $ (e.g. 90000)"
                  type="number"
                  style={{flex:1,background:"#f8fafc",border:"2px solid #e2e8f0",borderRadius:12,padding:"10px 12px",fontSize:13,color:"#0f172a"}}
                  onFocus={e=>e.target.style.borderColor="#f59e0b"} onBlur={e=>e.target.style.borderColor="#e2e8f0"}/>
                <button onClick={addAlert} disabled={!alertCoin||!alertPrice}
                  style={{background:"linear-gradient(135deg,#f59e0b,#d97706)",color:"#fff",border:"none",borderRadius:12,padding:"10px 16px",fontWeight:700,fontSize:13,cursor:"pointer",fontFamily:"'Inter',sans-serif",whiteSpace:"nowrap",boxShadow:"0 4px 14px rgba(245,158,11,.35)"}}>
                  🔔 Set
                </button>
              </div>
              {alertMsg&&<div style={{background:"#f0fdf4",border:"1px solid #6ee7b7",borderRadius:10,padding:"8px 12px",fontSize:12,color:"#059669",fontWeight:600,marginBottom:10}}>{alertMsg}</div>}
              {alerts.length>0&&(
                <div>
                  <div style={{fontSize:10,color:"#94a3b8",fontWeight:600,marginBottom:8,letterSpacing:.5}}>ACTIVE ALERTS</div>
                  {alerts.map(a=>(
                    <div key={a.id} style={{display:"flex",alignItems:"center",justifyContent:"space-between",background:"#fffbeb",border:"1px solid #fde68a",borderRadius:10,padding:"8px 12px",marginBottom:6}}>
                      <div>
                        <span className="mono" style={{fontWeight:700,fontSize:13}}>{a.coin}</span>
                        <span style={{fontSize:11,color:"#d97706",margin:"0 6px"}}>{a.type==="above"?"⬆️ Upar":"⬇️ Neeche"}</span>
                        <span className="mono" style={{fontWeight:700,fontSize:12}}>${a.price.toLocaleString()}</span>
                      </div>
                      <button onClick={()=>removeAlert(a.id)}
                        style={{background:"#fef2f2",border:"1px solid #fecaca",borderRadius:8,padding:"3px 8px",cursor:"pointer",fontSize:11,color:"#dc2626",fontWeight:600}}>✕</button>
                    </div>
                  ))}
                  <div style={{fontSize:10,color:"#94a3b8",marginTop:6,textAlign:"center"}}>
                    💡 Jab website open ho aur price target pe ho — alert dikhega
                  </div>
                </div>
              )}
            </div>
            ...
            {/* Screenshot Analysis */}
            <div id="screenshot-section" className="hov" style={{...CARD}}>
            ...
            {/* Mujhe Bata */}
            <div id="advisor-section" className="hov" style={{...CARD}}>
            ...
            {/* Mera Budget */}
            <div id="budget-section" className="hov" style={{...CARD}}>
