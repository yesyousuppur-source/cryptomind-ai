...
export default function Home() {
  ...
  const analyze = async () => {
    ...
    if (cache.current[sym] && now - cache.current[sym].ts < 60000) {
      ...
      setResult(d); setScamInfo(detectScam(d)); callAI(d); return;
    }
  ...
  const callAI = async (d) => {
    ...
    try {
      const r = await fetch("/api/ai", {
        ...
        body: JSON.stringify({
          mode: "analysis", language: "english",
  ...
  const whatIf = (() => {
    ...
    return [
      { label:"+50%", mult:1.5, color:"#059669" },
      { label:"+20%", mult:1.2, color:"#10b981" },
      { label:"+10%", mult:1.1, color:"#34d399" },
      { label:"-10%", mult:0.9, color:"#f87171" },
  ...
  const inp = { background: T.inputBg, border: `1px solid ${T.border2}`, borderRadius: 8, padding: "10px 13px", fontSize: 13, color: T.text, width: "100%", transition: "border-color .2s" };
  const card = { background: T.cardBg, border: `1px solid ${T.border}`, borderRadius: 12, padding: "16px", marginBottom: 10, boxShadow: T.shadow };
  ...
  return (
    <main style={{ fontFamily: "'Space Grotesk','Segoe UI',sans-serif", background: T.pageBg, minHeight: "100vh", color: T.text }}>
      ...
      <div style={{ maxWidth: 680, margin: "0 auto", padding: "28px 16px 48px", position: "relative" }}>
        ...
        <div style={{ display:"flex", gap:8, marginBottom:10 }}>
          <input
            ...
            style={{ flex:1, ...inp, padding:"13px 16px", fontSize:14, borderRadius:10 }}
          />
