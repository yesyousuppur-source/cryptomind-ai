...
const SectionHeader = ({icon, title, subtitle}) => (
  <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:16}}>
    <div style={{width:42,height:42,background:"linear-gradient(135deg,#ecfdf5,#d1fae5)",borderRadius:12,display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,border:"1px solid #6ee7b7",flexShrink:0}}>
      {icon}
    </div>
    <div>
...
export default function Home() {
  ...
  const fetchTop5=async()=>{
    ...
    try{
      ...
      const j=await r.json(); setTop5(j);
  ...
  const analyzeScreenshot=async()=>{
    ...
    try{
      ...
      setSsText(j.text || "Could not analyze. Please try again.");
      setSsLoad(false);
  ...
  const getPersonalAdvice=async()=>{
    ...
    try{
      const r=await fetch("/api/ai",{method:"POST",headers:{"Content-Type":"application/json"},
        body:JSON.stringify({mode:"personal",language:"english",userSituation:situation})});
  ...
  return (
    <main style={{fontFamily:"'Inter',sans-serif",background:"#f0fdf8",minHeight:"100vh",color:"#0f172a"}}>
      <style>{`
        ...
        .btn{background:linear-gradient(135deg,#10b981,#059669);color:#fff;border:none;border-radius:12px;font-weight:700;cursor:pointer;transition:all .2s;box-shadow:0 4px 16px rgba(16,185,129,.35)}
        ...
        .inp-focus:focus{border-color:#10b981!important}
        .upload-zone{border:2px dashed #6ee7b7;border-radius:16px;padding:28px;text-align:center;cursor:pointer;transition:all .2s;background:linear-gradient(135deg,#f0fdf4,#ecfdf5)}
      ...
      <div style={{position:"fixed",inset:0,overflow:"hidden",pointerEvents:"none",zIndex:0}}>
        ...
        <div style={{position:"absolute",inset:0,backgroundImage:"radial-gradient(rgba(16,185,129,.1) 1px,transparent 1px)",backgroundSize:"32px 32px",opacity:.4}}/>
      </div>
      ...
      <div style={{maxWidth:700,margin:"0 auto",padding:"36px 16px 56px",position:"relative",zIndex:1}}>
        ...
        <div style={{textAlign:"center",marginBottom:32}}>
          ...
          <div style={{display:"flex",justifyContent:"center",gap:6,marginTop:14,flexWrap:"wrap"}}>
            {["⚡ Real-time","🤖 YES YOU PRO AI","🛡️ Scam Detector","📸 Screenshot AI","🎯 Personal Advisor","🏆 Top 5 Picks"].map((p,i)=>(
              <span key={i} style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:20,padding:"4px 12px",fontSize:11,color:"#475569",fontWeight:500,boxShadow:"0 1px 4px rgba(0,0,0,.05)"}}>{p}</span>
            ))}
          </div>
        </div>
        ...
        {fgInfo&&fg&&(
          <div className="hov" style={{...CARD}}>
            <div style={{position:"absolute",top:0,left:0,right:0,height:3,background:`linear-gradient(90deg,${fgInfo.color},${fgInfo.color}66,transparent)`,borderRadius:"20px 20px 0 0"}} />
            ...
            <div style={{display:"flex",alignItems:"center",gap:16}}>
              <div className="float" style={{fontSize:44}}>{fgInfo.emoji}</div>
              <div style={{flex:1}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:10}}>
                  <div>
                    <div style={{fontSize:20,fontWeight:800,color:fgInfo.color}}>{fgInfo.label}</div>
                    <div style={{fontSize:11,color:"#94a3b8",marginTop:1}}>Market sentiment</div>
                  </div>
                ...
                <div style={{height:12,borderRadius:8,background:"linear-gradient(90deg,#dc2626,#f59e0b 35%,#fbbf24 50%,#34d399 70%,#10b981)",position:"relative",boxShadow:"0 2px 8px rgba(0,0,0,.08)"}}>
            ...
            <div style={{marginTop:12,background:fgInfo.bg,borderRadius:10,padding:"10px 14px",fontSize:12,color:"#475569",fontWeight:500}}>
        ...
        <div className="hov" style={{...CARD,position:"relative",overflow:"hidden"}}>
          ...
          <SectionHeader icon="🏆" title="Daily Top 5 Picks" subtitle="Real-time AI analysis — strongest buy signals today"/>
          {!top5Fetched ? (
            <div style={{textAlign:"center",padding:"8px 0 4px"}}>
              <p style={{fontSize:13,color:"#64748b",marginBottom:14,fontWeight:500}}>
                25 top coins scan kiye jaayenge — RSI, MA50, MA200 se real analysis hogi
              </p>
              <button className="btn" onClick={fetchTop5} style={{padding:"12px 28px",fontSize:14,borderRadius:14}}>
                🔍 Scan Top Coins Now
              </button>
            </div>
          ) : top5Load ? (
            <div style={{textAlign:"center",padding:"20px 0"}}>
              ...
              <p style={{fontSize:12,color:"#64748b"}}>25 coins scan ho rahi hain…</p>
              <p className="mono" style={{fontSize:10,color:"#94a3b8",marginTop:4}}>RSI · MA50 · MA200 · Volume calculating…</p>
            </div>
          ) : top5?.coins?.length > 0 ? (
            <div>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                <span style={{fontSize:11,color:"#94a3b8"}}>Updated: {new Date(top5.updatedAt).toLocaleTimeString("en-IN")}</span>
                <button onClick={fetchTop5} style={{background:"#f0fdf4",border:"1px solid #6ee7b7",borderRadius:20,padding:"4px 12px",fontSize:11,color:"#059669",fontWeight:600,cursor:"pointer"}}>
                  🔄 Refresh
                </button>
              </div>
              <div style={{display:"flex",flexDirection:"column",gap:10}}>
                {top5.coins.map((coin,i)=>(
                  <div key={coin.symbol} style={{background:`linear-gradient(135deg,${coin.signalBg},#fff)`,border:"1px solid #e2e8f0",borderRadius:14,padding:"14px 16px",display:"flex",alignItems:"center",gap:14,boxShadow:"0 2px 12px rgba(0,0,0,.04)"}}>
                    {/* Rank */}
                    <div style={{width:32,height:32,borderRadius:"50%",background:i===0?"linear-gradient(135deg,#fbbf24,#f59e0b)":i===1?"linear-gradient(135deg,#94a3b8,#64748b)":i===2?"linear-gradient(135deg,#cd7c32,#b45309)":"linear-gradient(135deg,#e2e8f0,#cbd5e1)",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:900,fontSize:14,color:i<3?"#fff":"#64748b",flexShrink:0,boxShadow:i<3?"0 2px 8px rgba(0,0,0,.15)":"none"}}>
                      {i+1}
                    </div>
                    {/* Coin info */}
                    <div style={{display:"flex",alignItems:"center",gap:10,flex:1,minWidth:0}}>
                      <img src={coin.image} alt="" onError={e=>e.target.style.display="none"}
                        style={{width:36,height:36,borderRadius:"50%",border:"2px solid #e2e8f0",flexShrink:0}}/>
                    ...
                    <div style={{textAlign:"right",flexShrink:0}}>
                      ...
                      <div style={{fontSize:11,color:parseFloat(coin.ch24)>=0?"#059669":"#dc2626",fontWeight:600}}>
                        {parseFloat(coin.ch24)>=0?"▲":"▼"}{Math.abs(parseFloat(coin.ch24)).toFixed(1)}%
                      </div>
                    </div>
                    {/* Score + Signal */}
                    <div style={{textAlign:"center",flexShrink:0}}>
                      <div style={{background:coin.signalBg,border:`1px solid ${coin.signalColor}44`,borderRadius:20,padding:"3px 10px",fontSize:11,color:coin.signalColor,fontWeight:800,marginBottom:4,whiteSpace:"nowrap"}}>
                        {coin.signal}
                      </div>
                      <div className="mono" style={{fontSize:10,color:"#94a3b8"}}>Score: {coin.score}/100</div>
                    </div>
                  </div>
                ))}
              </div>
              {/* Indicators summary */}
              <div style={{marginTop:12,background:"#f8fafc",borderRadius:12,padding:"12px 14px"}}>
                <div className="mono" style={{fontSize:10,color:"#94a3b8",marginBottom:8,letterSpacing:1}}>ANALYSIS BASIS</div>
                <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                  {["RSI (14-period)","MA50 position","MA200 trend","24h momentum","7d trend","Volume"].map(t=>(
                    <span key={t} style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:20,padding:"3px 10px",fontSize:10,color:"#475569",fontWeight:500}}>{t}</span>
                  ))}
                </div>
              </div>
            </div>
            ...
            <div style={{textAlign:"center",padding:"12px 0",color:"#94a3b8",fontSize:13}}>
              No strong buy signals found right now. Market may be bearish.
              <button onClick={fetchTop5} style={{display:"block",margin:"12px auto 0",background:"#f0fdf4",border:"1px solid #6ee7b7",borderRadius:20,padding:"6px 16px",fontSize:12,color:"#059669",fontWeight:600,cursor:"pointer"}}>Try Again</button>
            </div>
          )}
        </div>
          ...
          <div style={{display:"flex",gap:10,marginBottom:12}}>
            <div style={{flex:1,position:"relative"}}>
              ...
              <input value={query} onChange={e=>setQuery(e.target.value)} onKeyDown={e=>e.key==="Enter"&&analyze()}
                placeholder="Search: BTC, ETH, SOL, APT, DOGE, SUI…"
                className="inp-focus"
                style={{width:"100%",background:"#f8fafc",border:"2px solid #e2e8f0",borderRadius:12,padding:"13px 16px 13px 42px",fontSize:14,color:"#0f172a",transition:"border-color .2s"}}/>
            </div>
            <button onClick={analyze} disabled={loading} className="btn" style={{padding:"13px 24px",fontSize:14,borderRadius:12,whiteSpace:"nowrap"}}>
        ...
        {error&&(
          <div style={{background:"linear-gradient(135deg,#fff1f2,#fee2e2)",border:"1px solid #fca5a5",borderRadius:14,padding:"14px 18px",color:"#dc2626",marginBottom:14,fontSize:13,display:"flex",alignItems:"center",gap:10}}>
            <span style={{fontSize:18}}>⚠️</span><span className="mono">{error}</span>
          </div>
        )}
        ...
        {loading&&(
          <div style={{textAlign:"center",padding:"56px 0"}}>
            ...
            <p style={{color:"#64748b",fontSize:13,fontWeight:500}}>Fetching real-time data…</p>
        ...
        {result&&!loading&&(
          <div className="fadein">
            ...
            <div className="hov" style={{...CARD,display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:12}}>
              <div style={{display:"flex",alignItems:"center",gap:14}}>
                <div style={{position:"relative"}}>
                  <img src={result.image} alt="" onError={e=>e.target.style.display="none"} style={{width:50,height:50,borderRadius:"50%",border:"2px solid #e2e8f0",boxShadow:"0 4px 12px rgba(0,0,0,.08)"}}/>
            ...
            {/* Scam */}
            {scamInfo&&(
              <div style={{background:scamInfo.bg,border:`1px solid ${scamInfo.border}`,borderRadius:16,padding:"14px 18px",marginBottom:12}}>
                <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:scamInfo.flags.length?10:0}}>
                  ...
                  <div><div className="mono" style={{fontSize:10,color:"#94a3b8",letterSpacing:2}}>SCAM DETECTOR</div><div style={{fontWeight:700,fontSize:14,color:scamInfo.color}}>{scamInfo.verdict}</div></div>
                  <div style={{marginLeft:"auto",background:"rgba(255,255,255,.7)",borderRadius:12,padding:"6px 14px",border:`1px solid ${scamInfo.border}`}}>
                    <span style={{fontSize:11,color:scamInfo.color,fontWeight:700}}>Risk: {scamInfo.pts}/10</span>
                  </div>
                </div>
                {scamInfo.flags.length>0&&<div style={{display:"flex",flexWrap:"wrap",gap:5,marginBottom:8}}>{scamInfo.flags.map((f,i)=><span key={i} className="mono" style={{background:"rgba(255,255,255,.6)",border:`1px solid ${scamInfo.border}`,borderRadius:20,padding:"3px 10px",fontSize:10,color:scamInfo.color}}>{f}</span>)}</div>}
                {scamLoad&&<div style={{height:32,background:"rgba(255,255,255,.5)",borderRadius:8,animation:"shimmer 1.5s infinite"}}/>}
            ...
            <div style={{background:dc.bg,border:`2px solid ${dc.border}`,borderRadius:24,padding:"32px 24px",marginBottom:12,textAlign:"center",boxShadow:dc.glow,position:"relative",overflow:"hidden"}}>
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:6,justifyContent:"center"}}>
                {result.factors.slice(0,4).map((f,i)=><span key={i} className="mono" style={{background:"rgba(255,255,255,.6)",backdropFilter:"blur(8px)",border:"1px solid rgba(0,0,0,.08)",borderRadius:20,padding:"4px 12px",fontSize:10,color:"#475569",fontWeight:500}}>{f}</span>)}
              </div>
            </div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:12}}>
                  <div style={{position:"relative",width:52,height:52}}>
                    <svg viewBox="0 0 52 52" style={{transform:"rotate(-90deg)"}}>
                      ...
                      <circle cx="26" cy="26" r="22" fill="none" stroke={result.healthScore>=80?"#10b981":result.healthScore>=50?"#f59e0b":"#ef4444"} strokeWidth="4" strokeLinecap="round" strokeDasharray={`${(result.healthScore/100)*138.2} 138.2`} style={{transition:"stroke-dasharray .8s ease"}}/>
                    </svg>
              ...
              <div className="hov" style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:16,padding:"16px",boxShadow:"0 2px 12px rgba(0,0,0,.04)"}}>
                ...
                <div style={{fontSize:11,color:"#94a3b8",marginTop:2}}>Current sentiment</div>
              </div>
            </div>
            ...
            <div className="hov" style={{...CARD}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10}}>
                {[
                  {label:"📥 ENTRY ZONE",val1:fmt(result.entryLow),val2:fmt(result.entryHigh),sub:"to",c:"#059669",bg:"linear-gradient(135deg,#ecfdf5,#d1fae5)",br:"#6ee7b7"},
                  {label:"📤 EXIT TARGET",val1:fmt(result.exitTarget),sub:`+${(((result.exitTarget/result.price)-1)*100).toFixed(1)}% 🎯`,c:"#d97706",bg:"linear-gradient(135deg,#fffbeb,#fef3c7)",br:"#fde68a"},
                  {label:"🛑 STOP LOSS",val1:fmt(result.stopLoss),sub:"−6% limit",c:"#dc2626",bg:"linear-gradient(135deg,#fff1f2,#fee2e2)",br:"#fca5a5"},
                ].map((z,i)=>(
                  <div key={i} style={{background:z.bg,borderRadius:14,padding:"14px 10px",textAlign:"center",border:`1px solid ${z.br}`}}>
                    <div style={{fontSize:11,color:z.c,fontWeight:700,marginBottom:8}}>{z.label}</div>
                    <div className="mono" style={{fontSize:i===0?12:15,color:z.c,fontWeight:i===0?700:900}}>{z.val1}</div>
                    {i===0&&<><div style={{fontSize:10,color:"#94a3b8",margin:"3px 0"}}>{z.sub}</div><div className="mono" style={{fontSize:12,color:z.c,fontWeight:700}}>{z.val2}</div></>}
                    {i!==0&&<div style={{fontSize:11,color:z.c,fontWeight:700,marginTop:6,background:"rgba(255,255,255,.5)",borderRadius:20,padding:"2px 8px"}}>{z.sub}</div>}
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* YES YOU PRO AI */}
            <div className="hov" style={{background:"linear-gradient(135deg,#ecfdf5,#f0fdf4)",border:"1px solid #6ee7b7",borderRadius:20,padding:"18px",marginBottom:12,position:"relative",overflow:"hidden"}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
                ...
                <div><div style={{fontWeight:800,fontSize:13,color:"#065f46"}}>YES YOU PRO</div><div className="mono" style={{fontSize:9,color:"#059669",letterSpacing:1}}>AI ANALYSIS</div></div>
                {aiLoading&&<div style={{marginLeft:"auto",display:"flex",gap:4,alignItems:"center"}}><span style={{fontSize:11,color:"#059669",marginRight:4}}>Analyzing</span>{[0,1,2].map(i=><div key={i} style={{width:5,height:5,borderRadius:"50%",background:"#10b981",animation:`blink 1.2s ${i*.2}s infinite`}}/>)}</div>}
              </div>
              {aiText?<p style={{fontSize:13,color:"#166534",lineHeight:1.7,whiteSpace:"pre-line",fontWeight:500}}>{aiText}</p>
        ...
        <div style={{position:"relative",margin:"8px 0 20px"}}>
          ...
          <div style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",background:"#f0fdf8",padding:"0 16px"}}>
            <span style={{background:"linear-gradient(135deg,#10b981,#059669)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",fontWeight:900,fontSize:14,letterSpacing:.5}}>✦ TOOLS ✦</span>
          </div>
        </div>

        {/* 📸 SCREENSHOT ANALYSIS */}
        <div className="hov" style={{...CARD}}>
          <SectionHeader icon="📸" title="Screenshot Analysis" subtitle="Upload your exchange screenshot — AI will analyze your portfolio"/>
          <div className="upload-zone" onClick={()=>fileRef.current?.click()}>
            {ssPreview ? (
              <div>
                <img src={ssPreview} alt="preview" style={{maxHeight:180,borderRadius:10,marginBottom:10,maxWidth:"100%",boxShadow:"0 4px 16px rgba(0,0,0,.1)"}}/>
                <p style={{fontSize:12,color:"#059669",fontWeight:600}}>✓ Image ready — click Analyze</p>
              </div>
            ) : (
              <div>
                <div style={{fontSize:40,marginBottom:10}}>📷</div>
                <p style={{fontSize:14,color:"#059669",fontWeight:700,marginBottom:4}}>Upload Screenshot</p>
                <p style={{fontSize:12,color:"#64748b"}}>WazirX, CoinDCX, Binance — any exchange screenshot</p>
                <p className="mono" style={{fontSize:10,color:"#94a3b8",marginTop:6}}>JPG, PNG supported · Click to browse</p>
              </div>
            )}
          </div>
          ...
          {ssFile&&(
            <div style={{marginTop:10,display:"flex",gap:8}}>
              <button className="btn" onClick={analyzeScreenshot} disabled={ssLoad} style={{flex:1,padding:"12px",fontSize:14,borderRadius:12}}>
              ...
              <button onClick={()=>{setSsFile(null);setSsPreview(null);setSsText("");if(fileRef.current)fileRef.current.value="";}}
                style={{background:"#f8fafc",border:"1px solid #e2e8f0",borderRadius:12,padding:"12px 16px",cursor:"pointer",fontSize:13,color:"#64748b"}}>
                ✕ Clear
              </button>
            </div>
          )}
          {ssLoad&&<div style={{height:100,background:"linear-gradient(135deg,#f0fdf4,#dcfce7)",borderRadius:12,animation:"shimmer 1.5s infinite",marginTop:12}}/>}
          {ssText&&(
            <div style={{marginTop:12,background:"linear-gradient(135deg,#ecfdf5,#f0fdf4)",border:"1px solid #6ee7b7",borderRadius:14,padding:"16px"}}>
              <p style={{fontSize:13,color:"#166534",lineHeight:1.75,whiteSpace:"pre-line",fontWeight:500}}>{ssText}</p>
            </div>
          )}
        </div>

        {/* 🎯 MUJHE BATA — Personal Advisor */}
        <div className="hov" style={{...CARD}}>
          <SectionHeader icon="🎯" title="Mujhe Bata — Personal Advisor" subtitle="Apni situation likho — AI tumhare liye personalized advice dega"/>
          <div style={{marginBottom:10}}>
            <textarea value={situation} onChange={e=>setSituation(e.target.value)}
              placeholder={`Example: "Maine ₹10,000 DOGE mein lagaye hain, 40% loss mein hoon, kya karu?"\n\nYa: "Mere paas ₹5000 hain, Bitcoin buy karu ya wait karu?"`}
              rows={4}
              style={{width:"100%",background:"#f8fafc",border:"2px solid #e2e8f0",borderRadius:12,padding:"12px 14px",fontSize:13,color:"#0f172a",resize:"vertical",lineHeight:1.6,fontFamily:"'JetBrains Mono',monospace",transition:"border-color .2s"}}
              onFocus={e=>e.target.style.borderColor="#10b981"}
              onBlur={e=>e.target.style.borderColor="#e2e8f0"}
            />
          </div>
          <button className="btn" onClick={getPersonalAdvice} disabled={adviceLoad||!situation.trim()} style={{width:"100%",padding:"13px",fontSize:14,borderRadius:12}}>
            {adviceLoad?<span style={{display:"inline-block",animation:"spin .8s linear infinite"}}>⟳ Thinking…</span>:"🤖 Get Personal Advice"}
          </button>
          {adviceLoad&&<div style={{height:120,background:"linear-gradient(135deg,#f0fdf4,#dcfce7)",borderRadius:12,animation:"shimmer 1.5s infinite",marginTop:12}}/>}
          {adviceText&&(
            <div style={{marginTop:12,background:"linear-gradient(135deg,#ecfdf5,#f0fdf4)",border:"1px solid #6ee7b7",borderRadius:14,padding:"16px"}}>
              <p style={{fontSize:13,color:"#166534",lineHeight:1.8,whiteSpace:"pre-line",fontWeight:500}}>{adviceText}</p>
            </div>
          )}
        </div>
        ...
        {/* 🎯 MERA BUDGET */}
        <div className="hov" style={{...CARD}}>
          <SectionHeader icon="💰" title="Mera Budget" subtitle="Budget daalo — AI best coins suggest karega"/>
          <div style={{display:"flex",gap:8,marginBottom:10}}>
            ...
            <input value={budgetAmt} onChange={e=>setBudgetAmt(e.target.value)}
              ...
              className="inp-focus"
              style={{flex:1,...INP}}/>
            <button className="btn" onClick={callBudgetAI} disabled={budgetLoad||!budgetAmt} style={{padding:"11px 20px",fontSize:13,borderRadius:12,whiteSpace:"nowrap"}}>
              {budgetLoad?"⟳":"Ask AI 🤖"}
            </button>
          </div>
          ...
          {budgetText&&<div style={{background:"linear-gradient(135deg,#ecfdf5,#f0fdf4)",border:"1px solid #6ee7b7",borderRadius:14,padding:"14px 16px"}}><p style={{fontSize:13,color:"#166534",lineHeight:1.75,whiteSpace:"pre-line",fontWeight:500}}>{budgetText}</p></div>}
        </div>

        {/* 💡 WHAT-IF */}
        {result&&(
          <div className="hov" style={{...CARD}}>
            <SectionHeader icon="💡" title="What-If Simulator" subtitle="Profit & loss scenarios dekho"/>
            <input value={investAmt} onChange={e=>setInvestAmt(e.target.value)}
              placeholder="Investment in $ (e.g. 500)" type="number"
              className="inp-focus"
              style={{...INP,marginBottom:12}}/>
            {whatIf&&(
              <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:8}}>
                {whatIf.map((r,i)=>(
                  <div key={i} style={{background:r.c2,border:`1px solid ${r.c3}`,borderRadius:12,padding:"12px 6px",textAlign:"center",transition:"transform .15s",cursor:"default"}}
        ...
        {/* 🩹 LOSS RECOVERY */}
        <div className="hov" style={{...CARD}}>
          <SectionHeader icon="🩹" title="Loss Recovery Planner" subtitle="Recovery plan calculate karo"/>
          <input value={lossAmt} onChange={e=>setLossAmt(e.target.value)}
            placeholder="Your loss % (e.g. 30 = lost 30%)" type="number" min="1" max="99"
            className="inp-focus"
            style={{...INP,marginBottom:12}}/>
          {lossRec&&(
            <div>
              ...
              {[
                ...
                <div key={i} style={{display:"flex",gap:12,alignItems:"flex-start",background:p.bg,border:`1px solid ${p.br}`,borderRadius:14,padding:"12px 14px",marginBottom:8}}>
                  <span style={{fontSize:20,flexShrink:0}}>{p.icon}</span>
                  <div><span style={{fontSize:12,color:p.c,fontWeight:800}}>{p.name}: </span><span style={{fontSize:12,color:"#475569",fontWeight:500}}>{p.plan}</span></div>
                </div>
              ))}
            </div>
          )}
          ...
          <div style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:16,padding:"18px 20px",boxShadow:"0 2px 10px rgba(0,0,0,.05)"}}>
            ...
            <div style={{borderTop:"1px dashed #e2e8f0",paddingTop:12,textAlign:"center"}}>
              <div className="mono" style={{fontSize:10,color:"#94a3b8",lineHeight:1.7}}>
                ...
                {" · "}
                <a href="mailto:Yesyousuppur@gmail.com"
                  style={{color:"#059669",textDecoration:"none",fontWeight:600}}>Yesyousuppur@gmail.com</a>
              </div>
            </div>
          </div>
