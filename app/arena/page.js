...
function ArenaPageInner() {
  ...
  const [quickAction, setQuickAction] = useState(null); // {coin, type} from portfolio click

  // ── READ URL PARAMS — from Market page click ─────────────────────────────
  useEffect(() => {
    const coin = searchParams?.get("coin");
    const action = searchParams?.get("action") || "buy";
    if (coin) {
      const upperCoin = coin.toUpperCase();
      // Add coin to list if not present
      if (!COINS.includes(upperCoin)) COINS.push(upperCoin);
      setSelCoin(upperCoin);
      setTradeType(action);
      setActiveTab("trade");
  ...
  return (
    <main style={{fontFamily:"'Inter',sans-serif",background:T.page,minHeight:"100vh",color:T.text}}>
      ...
      <div style={{maxWidth:700,margin:"0 auto",padding:"18px 16px 56px",position:"relative"}}>
        ...
        {/* ── TRADE TAB ── */}
        {activeTab==="trade" && (
          <div className="fadein" id="trade-section">
                  ...
                  {Object.entries(holdings).filter(([,qty])=>qty>0.000001).map(([coin,qty])=>{
                    ...
                    return(
                      <div key={coin} style={{background:"#f8fafc",border:`1px solid ${T.border}`,borderRadius:12,padding:"12px 14px",marginBottom:8}}>
                        ...
                        {/* P&L Row */}
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",background:pnl>=0?"#f0fdf4":"#fef2f2",borderRadius:8,padding:"6px 10px",marginBottom:8}}>
                        ...
                        <div style={{display:"flex",gap:8}}>
                          <button onClick={()=>{
                            ...
                            setTradeAmt("");
                            setActiveTab("trade");
                            setTimeout(()=>document.getElementById("trade-section")?.scrollIntoView({behavior:"smooth"}),100);
                          }} style={{flex:1,background:"linear-gradient(135deg,#10b981,#059669)",color:"#fff",border:"none",borderRadius:10,padding:"8px",fontWeight:700,fontSize:12,cursor:"pointer",fontFamily:"'Inter',sans-serif"}}>
                            📈 Buy More
                          </button>
                          <button onClick={()=>{
                            setSelCoin(coin);
                            ...
                            setActiveTab("trade");
                            setTimeout(()=>document.getElementById("trade-section")?.scrollIntoView({behavior:"smooth"}),100);
                          }} style={{flex:1,background:"linear-gradient(135deg,#ef4444,#dc2626)",color:"#fff",border:"none",borderRadius:10,padding:"8px",fontWeight:700,fontSize:12,cursor:"pointer",fontFamily:"'Inter',sans-serif"}}>
                            📉 Sell
                          </button>
                        </div>
                      </div>
...
export default function ArenaPage() {
  return (
    <Suspense fallback={
      <div style={{fontFamily:"'Inter',sans-serif",background:"#f0fdf8",minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center"}}>
        <div style={{textAlign:"center"}}>
          <div style={{fontSize:40,marginBottom:12}}>🏟️</div>
          <p style={{color:"#64748b"}}>Arena load ho raha hai...</p>
        </div>
      </div>
    }>
      <ArenaPageInner/>
