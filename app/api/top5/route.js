...
export async function GET() {
  ...
  try {
    ...
    for (let i = 0; i < topCoins.length; i++) {
      ...
      // Label based on score — no hard cutoffs
      let signal, signalColor, signalBg, risk;
      if (analysis.score >= 72) {
        signal = "STRONG BUY"; signalColor = "#059669"; signalBg = "#ecfdf5"; risk = "Low";
      } else if (analysis.score >= 60) {
        signal = "BUY"; signalColor = "#10b981"; signalBg = "#f0fdf4"; risk = "Medium";
      } else if (analysis.score >= 48) {
        signal = "WATCH"; signalColor = "#d97706"; signalBg = "#fffbeb"; risk = "High";
      } else {
        signal = "CAUTION"; signalColor = "#dc2626"; signalBg = "#fef2f2"; risk = "Very High";
      }
      ...
      scored.push({
        ...
        risk, signal, signalColor, signalBg,
        rsi:    analysis.rsi,
        ...
        slPct: tpsl.slPct, tp1Pct: tpsl.tp1Pct,
        tp2Pct: tpsl.tp2Pct, tp3Pct: tpsl.tp3Pct,
        rrRatio: tpsl.rrRatio,
      });
    }

    // Always show top 5 by score — no hard filters
    scored.sort((a,b) => b.score - a.score);
    const top = scored.slice(0, 5);

    const avgScore = top.length > 0 ? top.reduce((s,c)=>s+c.score,0)/top.length : 0;
    const result = {
      coins: top,
      scanned: topCoins.length,
      qualified: top.length,
      isWeakMarket: avgScore < 55,
      message: top.length === 0 ? "no_signal" : "normal",
